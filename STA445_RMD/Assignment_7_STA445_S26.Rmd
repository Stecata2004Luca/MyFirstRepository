...
ggplot(trees,
       aes(x= Height,
           y = Volume))+
  geom_point() +
  annotate('rect', xmin=60, xmax=68, ymin=70, ymax=80, fill='white', color='black') +
  annotate('text', x=64, y = 77, 
           label = TeX("$\\hat{y}_i = -87.12 + 1.54x_i$", output="character"), 
           parse = TRUE ) +
  annotate('text', x=64, y =73, 
           label = TeX("$R^2 = 0.358$", output= "character"), 
           parse = TRUE ) +
...
```{r}
    my.titanic <- Titanic %>% as.data.frame()
```
