...
```{r}
my.survey$Year <- as.factor(my.survey$Year)
my.survey <- my.survey %>%
...
```{r}
my.survey$Gender <-  as.factor(my.survey$Gender) %>%
...
|  **Male**   |              |            |           |           |
```{r}
pivot_wider(my.combo, names_from = Year, values_from = n)
```
...
```
cbind does not work as it has two columns that are displaying the same information
```{r}
...
full_join(A, B, by = c("Name" = "First.Name") )
```
It is easier to use the dplyr join commands because the join commands do most of the work for you. 
...
```{r message=FALSE, warning=FALSE}
 my.flagstaff <- read_csv("https://raw.githubusercontent.com/BuscagliaR/STA_444_v2/master/data-raw/FlagMaxTemp.csv")
```
...
make the year month and day column one


```{r}

my.flagstaff <- my.flagstaff %>%
  filter(Year == 2005) %>%
  pivot_longer(cols = -c(Year, Month, ...1),
               names_to = "day_column",
               values_to = "Temp") %>%
  mutate(day_column = as.numeric(day_column),
         Date = as.Date( paste(Year, Month, day_column, sep = "-"),  
      format = "%Y-%m-%d"))
  
```

```{r message=FALSE, warning=FALSE}
ggplot(my.flagstaff, aes(x = Date, y = Temp)) +
  geom_line() +
  labs(
    title = "Daily Max Temp in Flagstaff, 2005",
    x = "Date",
    y = "Max Temp (°F)"
  )
```
...
```{r}
my.flights <- flights %>%
  filter(day ==  "14" & month == "2" & carrier =="VX") %>%
  select(c("carrier","dest", "dep_time", "hour", "minute"))

my.airport <- airports

my.airlines <- airlines

inner_join(my.flights, my.airport, by = c("dest" = "faa"))
```
...
derek.card <- Cards %>%
  filter(PersonID == 1) %>%
  select(c("CardID", "PersonID"))
derek.t <- Transactions %>%
  filter(CardID == '9876768717278723')

Retailers <- Retailers %>%
  select(c('RetailID', "Name"))

table_1 =  inner_join(derek.c, derek.card, by = "PersonID")
table_2 = inner_join(derek.t, Retailers, by = 'RetailID')
final_table = full_join(table_1, table_2, by = "CardID") 

final_table %>%
  select(c("CardID", "DateTime", "Amount", "Name"))
...
Cards<- Cards %>%
  mutate(Exp_DateTime = if_else(PersonID == 2, close_time, Exp_DateTime))


Cards <- Cards %>%
  add_row(CardID = "1111111111111111", PersonID = 2,Issue_DateTime = close_time, Exp_DateTime = ymd_hms('2032-10-15 9:28:21' ))
```
...
if( nrow(Valid_Cards) == 1){
  Transactions <- add_row(Transactions, CardID = card, RetailID = retailid, DateTime = datetime, Amount = amount)
}else{
  print('Card Denied')
}
```
...
Aubrey.card <- Cards %>%
  filter(PersonID == 1) %>%
  select(c("CardID", "PersonID"))
Aubrey.t <- Transactions %>%
  filter(CardID == '9876768717278723' | CardID == '1111111111111111')

Retailers <- Retailers %>%
  select(c('RetailID', "Name"))

table_1 =  inner_join(Aubrey.c, Aubrey.card, by = "PersonID")
table_2 = inner_join(Aubrey.t, Retailers, by = 'RetailID')
final_table = full_join(table_1, table_2, by = "CardID") 
